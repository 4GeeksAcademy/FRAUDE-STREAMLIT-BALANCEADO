...
if menu == "Predicción de Fraude":
    ...
    with st.form("Formulario de Datos"):  
        ...
        with col1:
            ...
            prev_address_months_count = st.number_input("Meses en Dirección Anterior", min_value=0, max_value=240, step=1, value=12)
            current_address_months_count = st.number_input("Meses en Dirección Actual", min_value=0, max_value=240, step=1, value=12)
            customer_age = st.number_input("Edad del Cliente", min_value=18, max_value=100, step=1, value=30)
            velocity_6h = st.number_input("Velocidad Transacción 6h", min_value=0.0, max_value=1000.0, step=1.0, value=10.0)
            velocity_24h = st.number_input("Velocidad Transacción 24h", min_value=0.0, max_value=1000.0, step=1.0, value=20.0)
            has_other_cards = st.radio("¿Tiene Otras Tarjetas?", ["No", "Sí"], index=0)
            foreign_request = st.radio("¿Solicitud Extranjera?", ["No", "Sí"], index=0)

        with col2:
            proposed_credit_limit = st.number_input("Límite de Crédito Propuesto", min_value=0.0, max_value=1000000.0, step=1000.0, value=5000.0)
            bank_branch_count_8w = st.number_input("Sucursales Bancarias 8 Semanas", min_value=0, max_value=20, step=1, value=5)
            credit_risk_score = st.number_input("Puntuación de Riesgo Crediticio", min_value=0, max_value=1000, step=1, value=300)
            keep_alive_session = st.number_input("Duración Sesión Activa (min)", min_value=0.0, max_value=1440.0, step=1.0, value=60.0)
            month = st.slider("Mes de la Transacción", min_value=1, max_value=12, step=1, value=1)
